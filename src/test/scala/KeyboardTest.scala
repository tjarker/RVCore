...
class KeyboardTest extends FreeSpec with ChiselScalatestTester{
  ...
  def readChar(dut: Keyboard) : Int = {
    read(dut.clock, dut.bus, addr+8)
  }
  def setActivated(dut: Keyboard, a: Int) : Unit = {
    write(dut.clock, dut.bus, addr+4, a)
  }
  def getActivated(dut: Keyboard) : Boolean = {
    read(dut.clock, dut.bus, addr+4) == 1
  }
  def hasChar(dut: Keyboard) : Boolean = {
    read(dut.clock, dut.bus, addr) == 1
  }
  ...
  "PS2 should pass" in {
    test(new Keyboard(addr)){ dut =>
      ...
      for(char <- values){
        //println(hasChar(dut))
        assert(hasChar(dut))
        //println(s"${char.toHexString} :: ${readChar(dut).toHexString}")
